...
local function countfile(filename)
	local f = io.open(filename,"r")
	local count = {}
	for line in f:lines() do
		local errcode = line:sub(10,13)
		local errmsg = line:sub(15,-1)
		if count[errcode] then
			count[errcode].errcode = errcode
			count[errcode].num = count[errcode].num+1
			if not (errmsg[1] == "") then
				count[errcode].errmsg = errmsg
			end
		else
			count[errcode] = {}
			count[errcode].errcode = errcode
	...
	for k,v in pairs(count) do
		print(string.format("%s;%-d;%s",v.errcode,v.num,v.errmsg))
	end
end
